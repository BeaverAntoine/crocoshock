...
def celery_all(ctx):
    with ctx.cd(project_path("api")):
        ctx.run("celery -A crocoshock worker -l DEBUG --beat", pty=True, replace_env=False)


@task()
def celery_worker(ctx):
    with ctx.cd(project_path("api")):
        ctx.run("celery -A crocoshock worker -l DEBUG", pty=True, replace_env=False)


@task()
def celery_beat(ctx):
    with ctx.cd(project_path("api")):
        ctx.run("celery -A crocoshock beat -l DEBUG", pty=True, replace_env=False)


@task()
def celery_clear_queue(ctx):
    with ctx.cd(project_path("api")):
        ctx.run("celery -A crocoshock purge -f", pty=True, replace_env=False)
