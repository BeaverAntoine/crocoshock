...
def compose_up(ctx, daemon=False):
    with ctx.cd(project_path("docker")):
        command = "docker-compose --file docker-compose-local.yml --project-name=crocoshock up"
        if daemon:
            command = f"{command} -d"
...
def compose_down(ctx, volumes=False):
    with ctx.cd(project_path("docker")):
        command = "docker-compose --file docker-compose-local.yml --project-name=crocoshock down"
        if volumes:
            command = f"{command} -v"
        ctx.run(command, pty=True, replace_env=False)
